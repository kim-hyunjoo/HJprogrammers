function solution(n, wires) {
    ...
    wires.forEach((wire,i) => {
        ...
        const startChild = BFS(splitWires, start); //start node의 자식 개수
        const endChild = BFS(splitWires, end); //end node의 자식 개수
        const gap = Math.abs(startChild - endChild); //두 전력망이 가진 송전탑 개수의 차이(절대값)
